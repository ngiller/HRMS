-- +goose Up
-- ============================================================
-- Migration 00052: Auto-calculate overtime and loans
-- ============================================================
-- Sekarang calculate_employee_payroll() secara otomatis membaca
-- lembur (overtime) yang sudah approved dan pinjaman (loans)
-- yang menggunakan metode payroll_deduction.
--
-- Changes:
-- 1. Query overtime_calculation untuk periode berjalan
-- 2. Tambah total overtime_pay otomatis
-- 3. Query pinjaman aktif dan potong installment
-- ============================================================
...
DECLARE
    ...
    -- NEW: Overtime & Loans auto-calculation
...
BEGIN
    ...
    -- ============================================================
    -- Get current period start and end date
    ...
    -- ============================================================
    -- OVERTIME: Auto-add approved overtime for the current payroll period
    -- ============================================================
    SELECT COALESCE(SUM(overtime_pay), 0)
    INTO v_auto_overtime_pay
    ...
    -- Update gross salary again since overtime_pay increased
    v_gross_salary := p_base_salary + p_daily_wage + v_total_allowances + p_overtime_pay + p_thr_amount + p_bonus_amount;
    -- ============================================================

    -- ============================================================
    -- LOAN: Auto-deduct active loans with payroll_deduction
    -- ============================================================
...
BEGIN
    ...
    -- ============================================================
    -- OVERTIME: Auto-add approved overtime since last payroll
    -- ============================================================
    SELECT COALESCE(SUM(overtime_pay), 0)
    INTO v_auto_overtime_pay
    FROM overtime_calculation
    WHERE employee_id = p_employee_id
      AND date > v_last_payroll_start::date
      AND date <= v_current_period_start;
