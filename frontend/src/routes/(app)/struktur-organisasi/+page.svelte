<script lang="ts">
	...
	async function load() {
		...
		errorMessage = '';
		try {
			const response = await api.getTree() as ApiResponse<OrgNode[]>;
			const data: OrgNode[] = response.data || [];
			tree = data;
			expandAllNodes(data);
			countStats(data);
		} catch (error: unknown) {
			errorMessage = (error as { message?: string }).message || 'Gagal memuat data struktur organisasi';
		} finally {
			isLoading = false;
		}
	}
...
<div class="w-full">
	...
	<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
		...
		{#if isLoading}
			<PulseLoader variant="text" count={6} />
