<script lang="ts">
	...
	const columnDefs: ColDef[] = [
		...
		},
		{
			...
			cellRenderer: (params: any) => {
				if (!params.value) return '';
				const label = scheduleTypeLabels[params.value] || params.value;
		...
		},
		{
			...
			valueFormatter: (params: any) => params.value != null ? params.value + ' jam' : '',
			headerClass: 'text-xs font-semibold text-gray-500 uppercase tracking-wider',
		...
		<div>
			<div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
				<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50/50">
					...
					<button onclick={() => { showDetail = false; detailItem = null; }} class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition cursor-pointer" aria-label="Tutup">
						<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
					</button>
				</div>
				<div class="px-6 py-5 space-y-3">
					<div class="grid grid-cols-2 gap-4 text-sm">
						<div><span class="text-gray-500">Tipe:</span> <span class="font-medium">{scheduleTypeLabels[detailItem.schedule_type] || detailItem.schedule_type}</span></div>
						<div><span class="text-gray-500">Jam/Minggu:</span> <span class="font-medium">{detailItem.weekly_hours} jam</span></div>
		...
		<div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
			...
			{:else if items.length === 0}
				<div class="py-16 text-center">
					...
					<h3 class="text-sm font-semibold text-gray-900 mb-1">Belum ada data jadwal kerja</h3>
					<p class="text-sm text-gray-500">{searchQuery ? 'Tidak ditemukan dengan kata kunci "' + searchQuery + '"' : 'Data jadwal kerja akan muncul setelah ditambahkan.'}</p>
				</div>
			{:else}
						...
						<div class="p-4 hover:bg-blue-50/40 transition-colors">
							<div class="flex items-center gap-3 mb-2">
								...
								<div class="flex-1 min-w-0">
									...
									<div class="text-xs text-gray-400">{scheduleTypeLabels[item.schedule_type] || item.schedule_type} &middot; {item.weekly_hours} jam/minggu</div>
								</div>
								<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ring-1 {item.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}">{item.is_active ? 'Aktif' : 'Nonaktif'}</span>
							</div>
						</div>
					{/each}
				</div>
