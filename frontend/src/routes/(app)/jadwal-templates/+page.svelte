...
import EmptyState from '$lib/components/EmptyState.svelte';
	...
	let form = $state<Form>({
		name: '', description: '', schedule_type: 'weekly',
		days: [] as DayEntry[],
	});
	let formError = $state('');
	...
	let detailItem = $state<Template | null>(null);
	...
	function initDays(scheduleType: string): DayEntry[] {
		const count = scheduleType === '6_day' ? 6 : (scheduleType === '5_day' || scheduleType === 'weekly' ? 5 : 5);
		return Array.from({ length: 7 }, (_, i) => ({
			day_of_week: i,
			start_time: i < count ? '08:00' : '',
			end_time: i < count ? '17:00' : '',
			break_start: '12:00',
			break_end: '13:00',
		...
		},
		{
			...
			cellRenderer: (params: AgGridCellParams<Template>) => {
				...
				const viewBtn = createActionButton(iconView(),
					...
					'Detail template', () => viewDetail(item));
				container.appendChild(viewBtn);
				const editBtn = createActionButton(iconEdit(),
					'p-1.5 rounded-lg text-gray-400 hover:text-[#1A56DB] hover:bg-blue-50 transition cursor-pointer',
					'Edit template', () => openEditForm(item));
				...
				if (hasPermission('work_schedule', 'delete')) {
					const deleteBtn = createActionButton(iconDelete(),
						...
						'Hapus template', () => confirmDelete(item.id, item.name));
	...
	function openCreateForm() {
		formTitle = 'Tambah Template Jadwal';
		editingId = null;
		form = { name: '', description: '', schedule_type: 'weekly', days: initDays('weekly') };
		formError = '';
		showForm = true;
	}

	async function openEditForm(item: Template) {
			...
			if (d) {
				const days = d.days && d.days.length > 0
					? d.days.map((day: DayEntry) => ({
						...
						late_tolerance_minutes: day.late_tolerance_minutes ?? 15,
						early_leave_tolerance: day.early_leave_tolerance ?? 15,
					}))
					: initDays(d.schedule_type || 'weekly');
				form = {
	...
	async function handleSave() {
		if (!form.name.trim()) { formError = 'Nama template harus diisi'; return; }
		...
		try {
			...
			const payload: Record<string, unknown> = {
				...
				days: activeDays.map(d => ({
					...
					end_time: d.end_time,
					break_start: d.break_start || '12:00',
					break_end: d.break_end || '13:00',
					late_tolerance_minutes: d.late_tolerance_minutes || 15,
					early_leave_tolerance: d.early_leave_tolerance || 15,
				})),
			};
	...
	function onTypeChange() {
		if (!editingId) {
			form.days = initDays(form.schedule_type);
		}
	}
</script>
...
<div class="w-full">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
		<div>
			<h1 class="text-2xl font-bold text-gray-900 tracking-tight">Template Jadwal</h1>
			<p class="text-sm text-gray-500 mt-0.5">Buat template jadwal reusable untuk dipakai banyak karyawan</p>
		</div>
	...
	{#if !showForm && !showDetail}
		<div class="bg-white border border-gray-200 rounded-xl px-5 py-3.5 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
			<div class="relative flex-1 max-w-md">
				...
				<input type="search" value={searchQuery} placeholder="Cari template jadwal..." oninput={onSearchInput} class="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] focus:bg-white transition placeholder:text-gray-400" />
			</div>
			<div class="text-xs text-gray-400">{total > 0 ? total + ' template ditemukan' : ''}</div>
		</div>
	{/if}
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1.5">
							...
							<select bind:value={form.schedule_type} onchange={onTypeChange} class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition bg-white">
					...
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1.5">
							Deskripsi
							<textarea bind:value={form.description} rows="2" class="mt-1.5 w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition resize-none" placeholder="Deskripsi template (opsional)"></textarea>
						</label>
					</div>
				</div>

				<h3 class="text-sm font-semibold text-gray-800 pt-2 border-t border-gray-100">Jam Kerja per Hari</h3>
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						<thead>
							<tr class="border-b border-gray-100">
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 pr-2">Hari</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Mulai</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Selesai</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Ist. Mulai</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Ist. Selesai</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Telat (mnt)</th>
								<th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Pulang Cepat</th>
							</tr>
						</thead>
						<tbody>
							{#each form.days as day, i (i)}
								<tr class="border-b border-gray-50 hover:bg-gray-50/50 transition">
									<td class="py-2 pr-2 font-medium text-gray-700">{DAY_LABELS[i]}</td>
									<td class="py-2 px-2">
										<input type="time" bind:value={day.start_time} class="w-32 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
									<td class="py-2 px-2">
										<input type="time" bind:value={day.end_time} class="w-32 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
									<td class="py-2 px-2">
										<input type="time" bind:value={day.break_start} class="w-32 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
									<td class="py-2 px-2">
										<input type="time" bind:value={day.break_end} class="w-32 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
									<td class="py-2 px-2">
										<input type="number" bind:value={day.late_tolerance_minutes} min="0" max="120" class="w-20 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
									<td class="py-2 px-2">
										<input type="number" bind:value={day.early_leave_tolerance} min="0" max="120" class="w-20 px-2 py-1.5 border border-gray-200 rounded text-sm outline-none focus:ring-2 focus:ring-[#1A56DB]/20 focus:border-[#1A56DB] transition" />
									</td>
								</tr>
							{/each}
						</tbody>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
		...
		<div onclick={(e) => e.stopPropagation()} onkeydown={(e) => { if (e.key === 'Escape') cancelDelete(); }} role="dialog" tabindex="-1" aria-modal="true" class="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative">
			<div class="px-6 py-6 text-center">
				...
				<h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Template Jadwal</h3>
				...
				<p class="text-xs text-gray-400 mb-6">Template yang sudah dihapus tidak dapat dikembalikan.</p>
