<script lang="ts">
	...
	async function handleSave() {
		...
		try {
			...
			await companyApi.updateEmployeeTaxConfig(selectedEmployee.id, payload);
			configSuccess = 'Konfigurasi pajak berhasil disimpan';
			setTimeout(() => {
				showModal = false;
	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
		<div>
			<h1 class="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
				<span>📄 Pengaturan Pajak PPh 21 Karyawan</span>
			</h1>
			<p class="text-sm text-gray-500 mt-1">Kelola metode pengenaan PPh 21, persentase kustom, atau kustom nominal tetap per karyawan</p>
		</div>
	</div>
				...
				<table class="w-full text-left border-collapse">
					<thead>
						<tr class="border-b border-gray-100 bg-gray-50/50">
							...
							<th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Status PTKP</th>
					...
					<tbody class="divide-y divide-gray-50">
						{#each filteredEmployees as emp (emp.id)}
							<tr class="hover:bg-gray-50/40 transition">
								...
								<td class="px-6 py-4 text-sm text-gray-600 font-bold"><span class="bg-blue-50 text-blue-700 px-2 py-1 rounded-lg text-xs">{emp.ptkp_status || '-'}</span></td>
			...
			<div class="md:hidden space-y-3 px-4 py-3">
				{#each filteredEmployees as emp (emp.id)}
					<MobileCard
						...
						badges={emp.ptkp_status ? [{ label: emp.ptkp_status, color: 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' }] : undefined}
					>
						{#snippet footer()}
...
{#if showModal && selectedEmployee}
	...
	<div class="fixed inset-y-0 right-0 max-w-2xl w-full bg-white z-50 shadow-2xl flex flex-col transition-transform duration-300">
		...
		<div class="bg-gradient-to-r from-indigo-600 to-indigo-800 px-6 py-5 text-white flex items-center justify-between shadow-lg">
			<div>
				<h2 class="text-lg font-bold">Atur Override Pajak PPh 21</h2>
			...
			{:else}
				<div class="space-y-4">
					<div class="border border-gray-200 rounded-xl p-5 bg-gray-50/30 space-y-4">
						<div>
							<span class="text-sm font-bold text-gray-900">Metode Pengenaan Pajak</span>
							<p class="text-xs text-gray-400 mt-1">Secara default, pajak dihitung otomatis menggunakan Kategori TER berdasarkan status PTKP ({selectedEmployee.ptkp_status || 'TK0'}). Anda dapat memaksakan tarif persen tertentu atau nominal kustom tetap.</p>
						</div>
						...
						<div class="flex flex-col md:flex-row gap-4 md:items-center">
							...
							{#if employeeTaxConfig.override_type === 'rate'}
								<div class="w-full md:w-40">
									<label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Tarif Kustom</label>
									<div class="relative">
							...
							{:else if employeeTaxConfig.override_type === 'nominal'}
								<div class="w-full md:w-56">
									<label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Nominal Tetap</label>
									<div class="relative">
